  ...
  return (
    <div>
      ...
      <div className="flex md:flex-row flex-col justify-between md:mt-12  max-w-6xl mx-auto px-5 md:px-0 ">
        <input type="search"  placeholder="Search for a country..." className="border px-16 h-12 rounded-md " onChange={(e) => setCountry(e.currentTarget.value)}/>

      {/* Dropdown */}
      <div className="hs-dropdown relative inline-flex md:mt-0 mt-8">
        <button id="hs-dropdown-default" type="button" className="hs-dropdown-toggle py-3 px-4 inline-flex justify-center items-center gap-2 rounded-md border font-medium bg-white text-gray-700 shadow-sm align-middle hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white focus:ring-blue-600 transition-all text-sm  dark:hover:bg-slate-800 dark:text-black dark:hover:text-white dark:focus:ring-offset-gray-800">
        ...
        <div className="hs-dropdown-menu transition-[opacity,margin] duration-[0.1ms] hs-dropdown-open:opacity-100 opacity-0 w-72 hidden z-10 mt-2 min-w-[15rem] bg-white shadow-md rounded-lg p-2 dark:bg-white  dark:divide-gray-700" aria-labelledby="hs-dropdown-default">
      ...
      {
        data.filter(item =>  country ? item.name  === country : item.name ).filter(item => continent ? item.region === continent : item.region).map((card) =><Flag card={card} key={card.name}/>)
      }
      </div>
    </div>
  )
